...
namespace YG
{
    public class LeaderboardYG : MonoBehaviour
    {
        ...
        [
            ...
            Range(0, 3),
            Tooltip(
                "Сколько показывать сотых (цифр после запятой)? (при использовании Time type)\n  Например:\n  0 = 00:00\n  1 = 00:00.0\n  2 = 00:00.00\n  3 = 00:00.000\nВы можете проверить это в Unity не прибегая к тестированию в WebGL!"
            )
        ]
        ...
        )
        {
            ...
            else if (_name == nameLB)
            {
                ...
                {
                    GameObject sampleContainer = transform
                    ...
                    for (int i = 0; i < sampleContainer.transform.parent.childCount; i++)
                        if (i != 0)
                            Destroy(sampleContainer.transform.parent.GetChild(i).gameObject);

                    if (entriesLB == "No data")
                    {
                        sampleContainer.transform
                            .Find("Rank")
                    ...
                    else
                    {
                        ...
                        {
                            if (i != 0)
                                sampleContainer = Instantiate(
                                    sampleContainer,
                                    sampleContainer.transform.parent
                                );

                            sampleContainer.transform
                                .Find("Rank")
                                .GetComponentInChildren<Text>()
                                .text = rank[i].ToString();
                            sampleContainer.transform
                                .Find("Name")
                                .GetComponentInChildren<Text>()
                                .text = CheckName(playersName[i]);

                            if (!timeTypeConvert)
                                sampleContainer.transform
                                    .Find("Score")
                                    .GetComponentInChildren<Text>()
                                    .text = scorePlayers[i].ToString();
                            else
                            {
                            ...
                            if (playerPhoto != PlayerPhoto.NonePhoto)
                            {
                                ImageLoadYG imageLoad = sampleContainer.transform
                                    .Find("Photo")
                                    .GetComponentInChildren<ImageLoadYG>();
                                if (photo[i] == "nonePhoto")
                                {
                                    if (isHiddenPlayerPhoto)
                                    {
                                        ...
                                        {
                                            imageLoad.rawImage.texture =
                                                isHiddenPlayerPhoto.texture;
        ...
        string CheckName(string origName)
        {
            if (origName != "anonymous")
                ...
                return YandexGame.Instance.infoYG.IsHiddenTextTranslate();
        }
        ...
        public static string TimeTypeConvertStatic(int score, int decimalSize)
        {
            ...
            string milSec =
                decimalSize == 0 ? "" : "." + result.Remove(0, result.Length - decimalSize);
