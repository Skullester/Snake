...
namespace YG
{
    ...
    public class InfoYG : ScriptableObject
    {
        ...
        [Tooltip(
            "Flush — определяет очередность отправки данных. При значении «true» данные будут отправлены на сервер немедленно; «false» (значение по умолчанию) — запрос на отправку данных будет поставлен в очередь."
        )]
        public bool flush;

        [ConditionallyVisible(nameof(saveCloud))]
        [Tooltip(
            "Синхронизировать облачные сохранения с локальными? Если localSaveSync = false при включенных облачных сохранениях, то локальные просто не будут использоваться."
        )]
        public bool localSaveSync = true;

        [ConditionallyVisible(nameof(saveCloud))]
        [Tooltip(
            "Интервал облачных сохранений.\nПри использовании метода сохранения (SaveProgress), сохранения будут происходить локально, если таймер не достиг значения (Save Cloud Interval. По умолчанию = 5). Если же таймер достиг интервала, то сохранения запишутся в облако.\nПри загрузке сохранений, будут загружены более актуальные данные (из локальных или облачных сохранений)."
        )]
        [Min(0)]
        ...
        public enum CallingLanguageCheck
        {
            FirstLaunchOnly,
            EveryGameLaunch,
        ...
        [Tooltip(
            "Домен с которого будет скачиваться перевод. Если у вас возникли проблемы с авто-переводом, попробуйте поменять домен."
        )]
        [ConditionallyVisible(nameof(LocalizationEnable))]
        ...
        [ConditionallyVisible(nameof(LocalizationEnable))]
        [Tooltip(
            "Настройки для метода локализации с помощью CSV файла. Это подразоумивает перевод по ключам всех текстов игры в таблице Excel или Google Sheets."
        )]
        ...
        [ConditionallyVisible(nameof(LocalizationEnable))]
        [Tooltip(
            "Вы можете скорректировать размер шрифта для каждого языка. Допустим, для Японского языка вы можете указать -3. В таком случае, если бы базовый размер был бы, например, 10, то для японского языка он бы стал равен 7."
        )]
        ...
        [
            Min(0),
            Tooltip(
                "Для более старых версий Unity требуется задержка старта SDK (задержка в кардах в секунду).\nСтавить задержку сдедует, если при запуске игры на Web сервере, после загрузки игры происходит краш или функции SDK не работают. В таком случае, обновите Unity до актуальной версии, либо поставьте задержку (рекомедруется: 20).\nЕсли SDK успешно загружается, задержку ставить не требуется."
            )
        ]
        public int SDKStartDelay;

        [Tooltip(
            "Если данный параметр выключен, то статические баннеры будут отображаться только во время загрузки игры. Если данный параметр включен, то статические баннеры будут отображаться и во время загрузки игры и в самой игре они тоже будут присутствовать!"
        )]
        ...
        {
            if (i == 0)
                return fontsSizeCorrect.ru;
            else if (i == 1)
                return fontsSizeCorrect.en;
            else if (i == 2)
                return fontsSizeCorrect.tr;
            else if (i == 3)
                return fontsSizeCorrect.az;
            else if (i == 4)
                return fontsSizeCorrect.be;
            else if (i == 5)
                return fontsSizeCorrect.he;
            else if (i == 6)
                return fontsSizeCorrect.hy;
            else if (i == 7)
                return fontsSizeCorrect.ka;
            else if (i == 8)
                return fontsSizeCorrect.et;
            else if (i == 9)
                return fontsSizeCorrect.fr;
            else if (i == 10)
                return fontsSizeCorrect.kk;
            else if (i == 11)
                return fontsSizeCorrect.ky;
            else if (i == 12)
                return fontsSizeCorrect.lt;
            else if (i == 13)
                return fontsSizeCorrect.lv;
            else if (i == 14)
                return fontsSizeCorrect.ro;
            else if (i == 15)
                return fontsSizeCorrect.tg;
            else if (i == 16)
                return fontsSizeCorrect.tk;
            else if (i == 17)
                return fontsSizeCorrect.uk;
            else if (i == 18)
                return fontsSizeCorrect.uz;
            else if (i == 19)
                return fontsSizeCorrect.es;
            else if (i == 20)
                return fontsSizeCorrect.pt;
            else if (i == 21)
                return fontsSizeCorrect.ar;
            else if (i == 22)
                return fontsSizeCorrect.id;
            else if (i == 23)
                return fontsSizeCorrect.ja;
            else if (i == 24)
                return fontsSizeCorrect.it;
            else if (i == 25)
                return fontsSizeCorrect.de;
            else if (i == 26)
                return fontsSizeCorrect.hi;
            else
                return null;
        }
